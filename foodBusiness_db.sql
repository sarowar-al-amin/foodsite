...
-- Table Structure  for the announcment Table
...
-- Insert value to the announcement table
insert into 'announcement'('id', 'announcement_text','announcement_date') values
  (1,'Now we have a facebook page https://facebook/foodhub.com','2018-04-27'),
  (2,'Now we have a website https://foodhub/rajshahi.com','2018-05-27');
-- Create catagory table
create table if not exits 'catagory'(
  'id' int(11) not null AUTO_INCREMENT,
  'iteam_name' varchar(255) not null,
  primary key('id')
) ENGINE = InnoDB default CHARSET = latin1 AUTO_INCREMENT = 13;

--inserting data into catagory table
insert into 'catagory'('id','iteam_name') values
  (1, 'appetizers'),
  (2, 'sandwiches'),
  (3, 'grill'),
  (4, 'salads'),
...
-- Inserting data to the products table

insert into 'products'('id', 'product_name','product_price','product_description', 'catagory_id') values
...
create table if not exits 'profile'(
  'id' int(11) not null AUTO_INCREMENT,
...
-- Inserting data to the profile
insert into 'profile'('id','first_name','last_name','mail_address','email_address','catagory_id','user_id') values
(6, 'Windows 10', 'Blue', 'Micro Earth', 'micro@soft.com', 3, 6),
(7, 'Cristiano', 'Ronaldo', 'Rome', 'cr7@juve.com', 0, 7),
(8, 'Maxwell', 'The Great', 'Austrailia', 'max@well.com', 6, 8),
...
(7, 'football', 'c822c1b63853ed273b89687ac505f9fa'),
